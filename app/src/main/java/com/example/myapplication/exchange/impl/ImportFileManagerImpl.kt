...
import com.example.myapplication.exchange.dto.TowerWithAdditionals
...
//    private val testAdditional = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><TowerWithAdditionals><Tower><idtf>TF3093466</idtf><assetNum>A26622070</assetNum><stopSeq>1</stopSeq><km>644</km><pk>4</pk><m>81</m></Tower><Additionals><Additional><changeDate>Today</changeDate><type>1D3</type><number>4N</number></Additional><Additional><longitude>12.0</longitude><latitude>44.0</latitude><type>1D3</type><number>6N</number></Additional><Additional><longitude>12.0</longitude><latitude>44.0</latitude><type>1D3</type><number>8N</number></Additional><Additional><longitude>12.0</longitude><latitude>44.0</latitude><changeDate>Today</changeDate><type>1D3</type><number>10N</number></Additional><Additional><longitude>12.0</longitude><latitude>44.0</latitude><changeDate>Today</changeDate><type>1D3</type><number>2N</number></Additional></Additionals></TowerWithAdditionals>"
//
...
//
//        CoroutineScope(Dispatchers.IO).launch {
//            if (!passportImport(file, liveData, error)){
//                if (!towerWithAdditionalImport(file, liveData, error)){
...
//    }
//
//    private fun passportImport(file : File, liveData: MutableLiveData<WorkResult>, error : WorkResult.Error): Boolean{
//        try {
//            val jaxbContext = JAXBContext.newInstance(SectionCertificate::class.java)
//            val unmarshaller = jaxbContext.createUnmarshaller()
//            val sectionCertificate: SectionCertificate
//            try {
////                sectionCertificate = unmarshaller.unmarshal(file) as SectionCertificate
//                sectionCertificate = unmarshaller.unmarshal(testPassport.reader()) as SectionCertificate
//            } catch (exc: Exception){
//                throw RuntimeException("Wrong file type", exc)
//            }
//
...
//
//            val towersSize = sectionCertificate.Tower.size
//            val step = 100 / towersSize
//            var progress = 0
//
//
//            val towers = sectionCertificate.Tower.filter {
...
//        } catch (ex: Exception){
//            if (ex is RuntimeException && ex.message?.contains("Wrong file type") == true){
//                return false
//            }
//            error.addError(ex)
...
//    }
//
//    fun towerWithAdditionalImport(file : File, liveData: MutableLiveData<WorkResult>, error : WorkResult.Error): Boolean{
//        try {
//            val jaxbContext = JAXBContext.newInstance(TowerWithAdditionals::class.java)
//            val unmarshaller = jaxbContext.createUnmarshaller()
//            val towerWithAdditionals: TowerWithAdditionals
//            try {
////                towerWithAdditionals = unmarshaller.unmarshal(file) as TowerWithAdditionals
//                towerWithAdditionals = unmarshaller.unmarshal(testAdditional.reader()) as TowerWithAdditionals
//            } catch (exc: Exception){
//                throw RuntimeException("Wrong file type", exc)
//            }
//
//            if (towerWithAdditionals.Tower == null){
//                throw RuntimeException("Tower from file{$file} is empty!")
//            }
//
////            val passport_id = passportRepository.addPassport(Converter.fromXMLToPassport(sectionCertificate.Header))
//            val tower = towerRepository.findTowerWithParameters(QueryBuilder.buildSearchByAllFieldsQuery(towerWithAdditionals.Tower))
//                ?: throw RuntimeException("Tower must import first!")
//
//            val additionalSize = towerWithAdditionals.Additional.size
//            val step = 100 / additionalSize
//            var progress = 0
//
//
//            val additionals = towerWithAdditionals.Additional.map {
//                val additional = if (it.longitude != null && it.latitude != null){
//                    var coord = coordinateRepository.addCoordinate(
//                        Coordinate(longitude = it.longitude!!, latitude = it.latitude!!))
//                    if (coord == -1L){
//                        coord = coordinateRepository.getCoordinateByLongitudeAndLatitude(it.longitude!!, it.latitude!!)!!.coord_id
//                    }
//                    Converter.fromXMLAdditionalDtoToAdditional(it, tower.tower_id, coord)
//                } else {
//                    Converter.fromXMLAdditionalDtoToAdditional(it, tower.tower_id, null)
//                }
//
//                liveData.postValue(WorkResult.Progress(progress + step))
//                progress += step
